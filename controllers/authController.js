...
exports.signup=async (req,res)=>{
    ...
    try{
        ...
        res.status(201).json({
            success:true, message: "You account has been created successfully", result,
        });
    }catch(error){
        console.log(error);
    }
...
exports.signin=async(req,res)=>{
    ...
    try{
        ...
        res
            .cookie('Authorization', 'Bearer'+token,{expires:new Date(Date.now()+8 
  ...
  try{
    const{error,value}=changePasswordSchema.validate(({oldPassword, newPassword}));
        ...
        if(!result){
          return res 
            .status(401)
            .json({success: false, message:'Invalied credentails!'})
        }
...
exports.sendForgotPasswordCode = async (req, res) => {
    ...
    const codeValue = Math.floor(Math.random() * 1000000).toString();
		...
		if (info.accepted[0] === existingUser.email) {
			...
			await existingUser.save();
      console.log('Reset code (plain):', codeValue);
...
exports.verifyForgotPasswordCode = async (req, res) => {
    ...
    found: !!existingUser,
    code: existingUser?.forgotPasswordCode,
    validation: existingUser?.forgotPasswordCodeValidation
    });
  // Note: Only use '+' if the schema has select: false
		if (!existingUser) {
			return res
				.status(401)
				.json({ success: false, message: 'User does not exists!' });
